...
class ProximalGradientDescent:
    def __init__(self, tolerance=0.000001, learningRate=0.001, learningRate2=0.001, prev_residue=999999999999999999.,
    ...
    def run(self, model, str='t'):
        if self.mode == 'linear':
            self.learningRate = self.learningRate*1000
            #print "using linear"
        if self.mode=='lmm2':
            self.learningRate = self.learningRate*0.5
        if str == 't':
            #print "here"
            ...
            self.learningRate = self.learningRate*1e7
            epoch = 0
            residue = model.cost()
            theta = 1.
            theta_new = 0.
